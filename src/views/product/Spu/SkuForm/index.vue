          ...
          >
            <el-select placeholder="请选择" v-model="saleAttr.attrIdAndValueId">
              <el-option
                ...
                :value="`${saleAttr.baseSaleAttrId}:${saleAttrValue.saleAttrValueName}`"
...
export default {
  ...
  methods: {
    //获取SkuForm数据
    async getData(spuId) {
      const res = await this.$API.spu.reqSpu(spuId);
      if(res.code==200){
        //收集父组件给予的数据
        const spu = res.data;
        ...
        //获取销售属性的数据
        this.spuSaleAttrList = spu.spuSaleAttrList;
